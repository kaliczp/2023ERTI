...
for(tti in 1:10) {
    ...
    plot.recharge(tv.xts[from.to,1], recharge.xts[from.to], xaxs = "i", main = from.to, xaxs = "i", xlab ="")
    lines(as.zoo(sim3hr.xts), col = "magenta")
    par(fig = c(0.25,0.5,0,1), new = TRUE)
    plot.recharge(tv.xts[from.to,1], recharge.xts[from.to], xaxs = "i", main = from.to, xaxs = "i", ylim = long.rang, xlab = "")
    lines(as.zoo(sim3hr.xts), col = "magenta")
    ...
    plot.zoo(longterm.xts, xaxs = "i", main = longfrom.to, xaxs = "i", ylim = long.rang, xlab = "")
    lines(as.zoo(sim3hr.xts), col = "magenta")
    lines(as.zoo(sim6hr.xts), col = "red")
    lines(as.zoo(sim1day.xts), col = "green")
    lines(as.zoo(sim3day.xts), col = "blue")
